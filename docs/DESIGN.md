...
3. **Recording + live view**: the recorder writes raw pty bytes to `recording/*.log`. Bash/zsh hooks and the exec path add invisible private OSC markers for `cmd`/`out`/`end`, each stamped with a **secret per-session nonce**; the client-side renderer honours a marker only if the nonce matches (so operator-controlled command *output* cannot forge or hide a command line), sanitizes the command label of all control sequences, and turns the stream into a clean command log above a pinned status line. Full-screen TUI apps render inline as their collapsed screen content — they are deliberately NOT hidden/suppressed, since suppression keyed on operator output bytes could hide real output from the client and the recording. The bash hook captures the full typed line from history with a distilled bash-preexec state machine (armed only between the end of `PROMPT_COMMAND` and the next command, so any `PROMPT_COMMAND` shape — string, array, function — and pipeline stages are handled; falls back to `BASH_COMMAND` when history is disabled / `ignorespace`, never mislabeling as a prior command). The same renderer produces the saved readable `.txt`, so the live and saved rendered views never drift — while the raw `.log` is kept untouched as the complete record.
4. **Teardown** (Ctrl-C / window close / any exit): release the terminal scroll region → `kill -- -PGID` the tunnel and sshd groups (reaps every forked child — no orphans) → best-effort `deregister` at the relay → confirm the local port is unbound → after-snapshot + diff → save the raw recording (`.log`, complete) + a readable rendered `.txt` and any change-diff to `~/.floo-last-session` → wipe the workdir.
