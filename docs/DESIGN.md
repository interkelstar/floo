...
3. **Recording + live view**: the recorder writes raw pty bytes to a fixed-name `recording/session.raw` (fixed so the live console's `tail -F` can follow it even though it starts before the first operator connects). Bash/zsh hooks and the exec path add invisible private OSC markers for `prompt`/`cmd`/`out`/`end`, each stamped with a **secret per-session nonce**; the client-side renderer honours a marker only if the nonce matches (so operator-controlled command *output* cannot forge or hide a command line), sanitizes the command label of all control sequences, and turns the stream into a clean command log above a pinned status line. The rendered view drops the inter-command region (shell prompt + the echo of the typed command, bracketed by `prompt`→`cmd`) so each command shows once as `$ <cmd>`; full-screen TUI apps render inline (alt-screen is NOT suppressed — suppression keyed on operator output bytes could hide real output). The bash hook captures the full typed line from history with a distilled bash-preexec state machine (armed only between the end of `PROMPT_COMMAND` and the next command, so any `PROMPT_COMMAND` shape — string, array, function — and pipeline stages are handled; falls back to `BASH_COMMAND` when history is disabled / `ignorespace`, never mislabeling as a prior command). The same renderer produces the saved readable `session.log`, so the live and saved rendered views never drift — while the raw `session.raw` is kept untouched as the complete record.
4. **Teardown** (Ctrl-C / window close / any exit): release the terminal scroll region → `kill -- -PGID` the tunnel and sshd groups (reaps every forked child — no orphans) → best-effort `deregister` at the relay → confirm the local port is unbound → after-snapshot + diff → save the raw recording (`session.raw`, complete) + a readable rendered `session.log` and any change-diff to `~/.floo-last-session` → wipe the workdir.
